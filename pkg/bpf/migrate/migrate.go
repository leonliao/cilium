...
var fsBase = filepath.Join(defaults.DefaultMapRoot, "/tc/globals")

// Start the map migration process for a given ELF's maps.
...
func Start(elfPath string) error {
	...
	for name, spec := range coll.Maps {
		...
		if err := repinMap(name, spec); err != nil {
			return err
		}
	}
...
func repinMap(name string, spec *ebpf.MapSpec) error {
	file := filepath.Join(fsBase, name)
	...
	dest := file + statePending

	fmt.Printf("New version of map '%s' has different properties, re-pinning from '%s' to '%s'\n", name, file, dest)
...
// Finish finalizes the migration of an ELF's maps.
// Set revert to true if the migration needs to be rolled back.
func Finish(pathName string, revert bool) error {
	...
	for name, spec := range coll.Maps {
		...
		if err := finalizeMap(name, revert); err != nil {
			return err
		}
	}
...
func finalizeMap(name string, revert bool) error {
	// Attempt to open a 'pending' Map pin.
	file := filepath.Join(fsBase, name+statePending)
	...
	if revert {
		dest := filepath.Join(fsBase, name)
		fmt.Printf("Reverting map pin from '%s' to '%s' after failed migration\n", file, dest)
	...
	fmt.Printf("Unpinning map '%s' after successful recreation\n", file)
